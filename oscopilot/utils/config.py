...
from oscopilot.utils.utils import get_project_root_path, random_string
...
def setup_config():
    ...
    parser.add_argument(
        "--working_dir", type=str, default="working_dir", help="working dir path"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Enter the path of the files for your task or leave empty if not applicable",
    )
    parser.add_argument(
        "--max_repair_iterations",
        type=int,
        default=3,
        help="Sets the max number of repair attempts. Default is 3.",
    )
    parser.add_argument("--logging_filedir", type=str, default="log", help="log path")
    parser.add_argument(
        "--logging_filename", type=str, default="temp0325.log", help="log file name"
    )
    parser.add_argument(
        "--logging_prefix", type=str, default=random_string(16), help="log file prefix"
    )
    parser.add_argument(
        "--score", type=int, default=8, help="critic score > score => store the tool"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="Excel",
        help="The name of the software used for learning.",
    )
    parser.add_argument(
        "--package_name",
        type=str,
        default="openpyxl",
        help="The name of the package used for learning.",
    )
    parser.add_argument(
        "--demo_file_path",
        type=str,
        default=get_project_root_path() + "working_dir/Invoices.xlsx",
        help="Entering the path of the demo file helps you design the course, or leave it empty if not applicable.",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to the dataset cache folder",
    )
    parser.add_argument(
        "--level",
        type=int,
        default=1,
        help="Specifies the level of the GAIA dataset to use. Valid options are 1, 2, or 3",
    )
    parser.add_argument(
        "--dataset_type",
        type=str,
        default="test",
        help="Defines the type of dataset to use, either `validation` for development or `test` for testing purposes",
    )
    parser.add_argument(
        "--gaia_task_id", type=str, default=None, help="GAIA dataset task_id"
    )

    # for SheetCopilot
    parser.add_argument(
        "--sheet_task_id", type=int, default=1, help="sheet task dataset task id"
    )
...
def setup_pre_run(args):
    ...
    if args.query_file_path != "":
        task = task + "\nThe path of the files you need to use: {0}".format(
...
def self_learning_print_logging(args):
    ...
    Args:
        args (argparse.Namespace): The parsed command-line arguments.
    """
    task = "Your task is: Learn to use {0} to operate {1}".format(
        args.package_name, args.software_name
    )
    if args.demo_file_path != "":
        task = task + "\nThe path of the file helps you design the course: {0}".format(
            args.demo_file_path
        )
