...
class FridayAgent(BaseAgent):
    ...
    def __init__(self, planner, retriever, executor, Tool_Manager, config):
        ...
        self.executor = executor(
            prompt["execute_prompt"], tool_manager, config.max_repair_iterations
        )
    ...
    def run(self, task):
        ...
        while self.planner.sub_task_list:
            try:
                ...
                isTaskCompleted, isReplan = self.self_refining(
                    sub_task, execution_state
                )
                if isReplan:
                    continue
                if isTaskCompleted:
                    ...
                    print(
                        "{} not completed in repair round {}".format(
    ...
    def self_refining(self, tool_name, execution_state: ExecutionState):
        ...
        if node_type in ["Python", "Shell", "AppleScript"]:
            ...
            if judgement.status == "Replan":
                ...
                print(
                    "The new task list obtained after planning is: {}".format(
                        new_sub_task_list
                    )
                )
                isReplan = True
            elif judgement.status == "Amend":
                repairing_result = self.repairing(
                    tool_name,
                    code,
                    description,
                    state,
                ...
                if repairing_result.status == "Complete":
                    ...
                    print("The current task requires replanning...")
                    new_sub_task_list = self.replanning(
                        tool_name, repairing_result.critique
                    )
                    print(
                        "The new task list obtained after planning is: {}".format(
                            new_sub_task_list
    ...
    def executing(self, tool_name, original_task):
        ...
        if node_type == "QA":
            if self.planner.tool_num == 1:
                result = self.executor.question_and_answer_tool(
                    pre_tasks_info, original_task, original_task
                )
            else:
                result = self.executor.question_and_answer_tool(
                    pre_tasks_info, original_task, description
                )
            print(result)
            logging.info(result)
        else:
            ...
            try:
                if node_type == "API":
                    ...
                    code, invoke = self.executor.generate_tool(
                        tool_name, description, node_type, pre_tasks_info, relevant_code
                    )
            except Exception as e:
                print("api call failed:", str(e))
                return
            ...
            result = state.result
            logging.info(state)
            output = {"result": state.result, "error": state.error}
    ...
    def replanning(self, tool_name, reasoning):
        ...
        try:
            self.planner.replan_task(
                reasoning, tool_name, relevant_tool_description_pair
            )
        except Exception as e:
            print("api call failed:", str(e))
            return
